...
function _processVector(vector) {
  var array = [];
  for (var i = 0; i < vector.size(); ++i) array.push(vector.get(i));
  ...
  initUI() {
    ...
    document.body.addEventListener("drop", (e) => {
      ...
      switch (ext) {
        ...
        case "room":
          this.loadFile(this._loadRoom, file);
          break;
      }
    });
  ...
  loadFile(loader, file, data) {
    if (!data) {
      ...
      if (file.constructor == String) {
        ...
        LX.requestBinary(
          path,
          (data) => loader.call(this, path, data),
          (e) => {
            LX.popup(
              e.constructor === String ? e : `[${path}] can't be loaded.`,
              "Request Blocked",
              { size: ["400px", "auto"], timeout: 10000 },
            );
      ...
      reader.onload = (e) =>
        loader.call(this, file.name, e.target.result, file);
